...
class KeychainManager {
    ...
    func save(hiveConfiguration: HiveWalletConfiguration, for wallet: String) {
        do {
            let encoder = JSONEncoder()
            let encodedCredentials = try encoder.encode(hiveConfiguration)
            let query: [String: Any] = [
                kSecClass as String: kSecClassGenericPassword,
    ...
    func retriveHiveConfiguration(forWallet exchange: String) -> HiveWalletConfiguration? {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrAccount as String: exchange,
            kSecReturnData as String: kCFBooleanTrue!,
        ...
        if status == errSecSuccess, let retrievedData = dataTypeRef as? Data {
            do {
                let decoder = JSONDecoder()
                let credentials = try decoder.decode(HiveWalletConfiguration.self, from: retrievedData)
                return credentials
            } catch {
                print("Error: \(error.localizedDescription)")
                return nil
